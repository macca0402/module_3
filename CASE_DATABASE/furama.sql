CREATE DATABASE quan_ly_nghi_duong_Furama;

USE quan_ly_nghi_duong_Furama;
...
select *
from nhan_vien nv
where left(nv.ho_ten,1) in('H','T','K')
and length(nv.ho_ten)<15;

-- 3.Hiển thị thông tin của tất cả khách hàng có độ tuổi từ 18 đến 50 tuổi và có địa chỉ ở “Đà Nẵng” hoặc “Quảng Trị”.
SELECT * ,2023 - year(khach_hang.ngay_sinh) as age
FROM khach_hang where khach_hang.dia_chi like '% Đà Nẵng' or khach_hang.dia_chi like '% Quảng Trị'
...
from khach_hang 
join hop_dong on hop_dong.ma_khach_hang =khach_hang.ma_khach_hang where khach_hang.ma_loai_khach=1
...
select hop_dong.ma_hop_dong,nhan_vien.ho_ten,khach_hang.ho_ten,khach_hang.so_dien_thoai,dich_vu.ten_dich_vu, sum(hop_dong_chi_tiet.so_luong) as so_luong_dich_vu_di_kem
from hop_dong
...
order by tong_lan_su_dung desc 
limit 2;
...
nhan_vien.dia_chi,count(hop_dong.ma_nhan_vien) as tong_so_hop_dong
from nhan_vien
