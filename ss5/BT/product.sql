create database if not exists quan_ly_product;
use quan_ly_product;
...
explain select count(*) from product where product_amount =20;
...
delimiter //
   ...
   delimiter //
   create procedure add_product(in product_code varchar(50),product_name varchar(50),product_price float,product_amount int,product_description varchar(50),product_status nvarchar(10))
   begin
...
product_amount=product_amount,product_description=product_description,product_status=product_status where products.id=id;
END //
delimiter ;
